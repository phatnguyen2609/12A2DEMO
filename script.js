...
// ==========================================
// --- TỐI ƯU HIỆU NĂNG: DỪNG VIDEO COVER KHI KHUẤT MÀN HÌNH ---
// ==========================================
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (coverVideo && coverContainer) {
        const videoObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                // Nếu cover container không còn hiển thị trên màn hình (đã kéo xuống Tổ 1)
                if (!entry.isIntersecting) {
                    coverVideo.pause();
                } else {
                    // Nếu kéo lên trên và thấy lại cover
                    coverVideo.play().catch(e => console.log("Trình duyệt chặn autoplay: ", e));
                }
            });
        }, {
            root: null,
            threshold: 0 // Kích hoạt ngay khi vừa khuất hẳn hoặc vừa xuất hiện lại
        });
